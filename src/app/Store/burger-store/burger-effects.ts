...
export class BurgerEffects {
            ...
            mergeMap((payload:Burgers) =>
                this.burgerService.findAllBurgers().pipe(
                    map((payload) => { 
                        ...
                        return { type: ActionBurgerTypes.getAllBurgersSuccess,payload: payload['payload']};
                    }),
                    catchError(() => {
    ...
    createBurgersEffect$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            mergeMap((payload:Burgers) =>
                this.burgerService.saveBurger(payload['iconUrl'],payload['description'],payload['category']).pipe(
                    map(() => { 
   ...
   editBurgersEffect$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            mergeMap((payload) =>
                this.burgerService.editBurger(payload['id'],payload['iconUrl'],payload['description'],payload['category']).pipe(
                    map((response) => { 
   ...
   deleteBurgersEffect$ = createEffect(() => {
    ...
    constructor(private burgerService: BurgerService, private actions$: Actions,private router:Router) { }
}
